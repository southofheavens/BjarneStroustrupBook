...
Упражнение:
    Каждая программа должна содержать подсказки для пользователя. Пусть при вводе буквы H (в любом
    регистре) калькулятор выводит на экран инструкцию по эксплуатации.
*/
