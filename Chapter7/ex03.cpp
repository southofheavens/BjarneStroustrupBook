...
Упражнение:
    Реализуйте именованные константы, которые не могут изменять свои значения. Указание: в класс
    Variable необходимо добавить член, позволяющий различать константы и переменные, и проверять
    его в функции set_value(). Если хотите дать пользователю возможность объявлять собственные
    именованные константы (а не ограничиваться предопределенными константами pi и e), то 
    необходимо добавить соответствующее обозначение, например const pi = 3.14.
*/
                ...
                if (t.kind == '=')
                {
                    ...
                    for (Variable& elem : varTable)
                    {
                        if (elem.name == nameOfVariable)
                        {
                            ...
                            throw std::invalid_argument("Попытка присвоить константе новое значение!");
                        }
                    }
                    return newValue;
                }
