...
Упражнение:
    Модифицируйте функцию Token_stream::get() так, чтобы, обнаружив символ перехода на следующую
    строку, она возвращала Token(print). Это повлечет за собой реализацию просмотра пробельных
    символов и обработку символа '\n'. Для этого можно использовать стандартную библиотечную
    функцию isspace(ch), возвращающую значение true, если символ ch является пробельным.

    Есть проблема. Если ввести следующий код
    5+
    5
    программа ломается
*/
