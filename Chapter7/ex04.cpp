...
Упражнение:
    Функции get_value(), set_value(), is_declared() и define_name() работают с переменной var_table.
    Определите класс Symbol_table с членом var_table типа vector<Variable> и функциями-членами 
    get(), set(), is_declared() и define(). Перепишите калькулятор с использованием переменной
    типа Symbol_table.
*/
...
public:
    ...
    double Define(std::string var, double val, bool isLet);
private:
...
double SymbolTable::Define(std::string var, double val, bool isLet)
...
// заданымм выражением
{
    ...
    double d = Expression();
    symbolTab.Define(varName, d, isLet);
    return d;
}
        ...
        case name:
        {
            if(symbolTab.IsDeclared(t.name))
            {
                ...
                if (t.kind == '=')
                {
                    ...
                    symbolTab.Set(nameOfVariable, newValue);
                    return newValue;
                }
                else
                {
                    ts.PutBack(t);
                    return symbolTab.Get(nameOfVariable);
                }
            }
