/*
    Напишите программу, подсчитывающую количество слов в объекте класса Document. Предусмотрите две версии:
    одну, в которой слово - это "последовательность символов, разделенных пробельными символами", и вторую,
    в которой слово - это "неразрывная последовательность алфавитных символов". Например, при первом определении
    выражения alpha.numeric и as12b это слова, а при втором каждое из них рассматривается как два слова.
*/
...
// Слово - последовательность символов, разделенных пробельными символами
uint32_t CountOfWords1(Document& d)
{
    uint32_t countOfWords = 0;
    uint32_t countOfLetters = 0;
...
// Слово - неразрывная последовательность алфавитных символов
uint32_t CountOfWords2(Document& d)
{
    uint32_t countOfWords = 0;
    uint32_t countOfLetters = 0;
...
int main()
{
    ...
    std::cout << CountOfWords1(d) << '\n';
    std::cout << CountOfWords2(d) << '\n';
