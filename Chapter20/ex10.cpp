/*
    Напишите подсчитывающую слова программу, в которой пользователь мог бы сам задавать набор
...
uint32_t CountOfWords3(Document& d, const std::string& separators)
{
    uint32_t countOfWords = 0;
    uint32_t countOfLetters = 0;
    ...
    for (char l : d)
    {
        std::string::const_iterator sep = std::find(separators.begin(), separators.end(),l);
        if (sep == separators.end())
        {
            countOfLetters++;
        } else
        {
